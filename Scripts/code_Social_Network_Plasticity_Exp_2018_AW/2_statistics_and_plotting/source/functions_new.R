...
collective_analysis_no_rescal <- function(data_path=data_path){
  ...
  barplot_delta_collective_list <- list() #ADD
  ...
  for (i in 1:length(variable_list)){
    ...
    #plot
    barplot_delta_collective <- barplot_delta(dataset=data,predictor="treatment",post_hoc_outcomes=post_hoc_outcomes,stats_outcomes=stats_outcomes,i=i,type="collective",collective=T,plot_untransformed=T,diff_type="") #form_stat=NULL, #ADD (i, post_hoc_outcomes, stats_outcomes)
    print(barplot_delta_collective) #ADD
    
    barplot_delta_collective_list[[variable_list[i]]]        <- barplot_delta_collective #ADD
    #names(barplot_delta_collective_list[[i]]) <- variable_list[i] #ADD
    #ADD SAVING OF THE PLOT on single pdf file as done in plot_grooming file
  }
  rownames(stats_outcomes) <- NULL
  
  return(list(stats_outcomes=stats_outcomes,    post_hoc_outcomes=post_hoc_outcomes,  barplot_delta_collective_list=barplot_delta_collective_list)) #ADD
}
