...
plot_network(case="topology_comparison",which_to_draw=c("PreTreatment_observed","PostTreatment_observed")) #AW DOES NOT MAKE SENSE FOR ME, IT IS FOR PLOTTING A DISPLAY FIGURE
...
variable_list <- c("modularity","clustering","task_assortativity","efficiency") #AW CHECK FOR EXTRA NETWORK VARIABLES, NAMES SHOULD MATCH 
names(variable_list) <- c("modularity","clustering","assortativity","efficiency") #THE NAMES HERE CAN BE CHANGED, IT IS THE DISPLAYED NAMES IN PLOT
transf_variable_list <- c("none","none","none","none") #AW I SHOULD CHECK IF THE VARS NEED TO BE TRANSFORMED BEFORE DOING THIS
analysis <- list(variable_list=variable_list,
                 transf_variable_list=transf_variable_list,
                 violin_plot_param = list(c(1.5,0,-0.02,0.25,0.01),c(1.5,-0.02,-0.02,0.25,0.01),c(1.5,-0.02,-0.02,0.25,0.01),c(1.5,-0.02,-0.02,0.25,0.001))) #THEY HAVE TO BE SPECIFIED DEPENDING ON THE SIZE OF 
